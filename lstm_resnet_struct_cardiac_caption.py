...
def load_data():
    ...
    #train_cycle = trDataset['num_cycle']
    #train_patientId = trDataset['patientId']
    train_labels = np.abs(train_labels)

    test_images = tsDataset['images']
    test_labels = tsDataset['labels']
    #test_cycle = tsDataset['num_cycle']
    #test_patientId = tsDataset['patientId']
...
def struct_loss(predictions, targets):
    ...
    alpha = 0.5#change this to assign the weight to the structured loss functio
    beta = 0.5
    ...
    total_loss = alpha * loss_temp + beta* loss_mse.mean()
    return total_loss
...
def compute_caption_error(y_hat_frames, y_true_frames, frame_len):
    ...
    y_hat_dias = np.argmax(y_hat_frames, axis=1)
    y_true_dias = np.argmax(y_true_frames, axis=1)
...
def iterate_minibatches(images, targets, mean_images, batchsize,randomize='false'):
    indices = np.arange(len(targets))
    for start_idx in range(0, len(images) - batchsize + 1, batchsize):
        excerpt = indices[start_idx:start_idx+batchsize]
        if randomize == 'true':
	       excerpt = np.roll(excerpt, np.array(random.sample(excerpt,1)))    
...
def main(n=2, num_epochs=500, batch_size=90, model=None):
    print("Loading data...")
    data = load_data()
    ...
    lr = 0.005  # for skips 1, lr = 0.005; for depth_5, lr =0.0005
    ...
    num_fold = 10
    fold_r2_score = np.zeros(num_fold)
    fold_sys_error = np.zeros(num_fold)
    fold_dias_error = np.zeros(num_fold)
    fold_mse_error = np.zeros(num_fold)

    for indxFold in range(0, num_fold, 1):
        images = test_images
        labels = Y_test
        ...
        for batch in iterate_minibatches(images,
            ...
            if indxFold == 0:
                ...
                gt_line, = final_plot.plot(targets, 'b', marker='o', linewidth=2)
                predid_line, = final_plot.plot(predid, 'g', marker='o', linewidth=2)
                final_plot.legend((gt_line, predid_line), ('Ground Truth', 'Prediction'), fontsize=18)
                final_plot.title.set_text('')
                final_plot.set_xlabel('Num Frames', fontsize=20)
                final_plot.set_ylabel('Labels', fontsize=20)
                final_plot.tick_params(axis='both', which='both', labelsize=18)
        ...
        fold_r2_score[indxFold] = final_r2_score
        fold_sys_error[indxFold] = final_sys_error
        fold_dias_error[indxFold] = final_dys_error
        fold_mse_error[indxFold] = final_mse_err
        #print "Finished"
        print('Finished {:.0f} fold of Test randomisation \n'.format(indxFold))


    print("******* Final Testing Results on 30 fold ******")
    print("Mean Square Error:\t\t{:.3f}({:.3f})".format(np.mean(fold_mse_error), np.std(fold_mse_error)))
    print("R-2 Score:\t\t\t{:.3f}({:.3f})".format(np.mean(fold_r2_score), np.std(fold_r2_score)))
    print("Systolic Error:\t{:.3f}({:.3f})".format(np.mean(fold_sys_error), np.std(fold_sys_error)))
    print("Diastolic  Error:\t{:.3f}({:.3f})".format(np.mean(fold_dias_error),np.std(fold_dias_error)))
